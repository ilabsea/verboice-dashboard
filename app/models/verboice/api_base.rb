module Verboice::ApiBase
  ...
  module ClassMethods
    ...
    def update(id, action = nil, attributes={})
      object = self.new(attributes.merge(id: id))
      url = action.nil? ? "#{base_url}/#{id}" : "#{base_url}/#{id}/#{action}"
      response = Typhoeus::Request.put(url, body: embed(attributes), headers: {"content-type" => "application/x-www-form-urlencoded"})
